...
void merge_telemetry(double **telemetries,
  ...
  for(int i = 0; i < global_telemetry_current_size / 3; i++)
    for(int j = i + 1; j < global_telemetry_current_size / 3; j++)
      if(global_telemetry[3 * i] > global_telemetry[3 * j]) {
          swap(global_telemetry[3 * i], global_telemetry[3 * j]);
          swap(global_telemetry[3 * i + 1], global_telemetry[3 * j + 1]);
          swap(global_telemetry[3 * i + 2], global_telemetry[3 * j + 2]);
      }
  return;
}
